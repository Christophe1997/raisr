...
class RAISR:
    ...
    def train(self, lr_path: str, hr_path: str, sharpen: bool = True, dst="./model"):
        ...
        Q = np.zeros((self.angle_base, t2, d2, d2))
        V = np.zeros((self.angle_base, t2, d2, 1))
        H = np.zeros((self.angle_base, t2, d2, 1))
        ...
        def f(item):
            ...
            for idx1, patch in enumerate(patches):
                ...
                # conpute hash key j
                angle = gen_hash_key(patch, self.angle_base, self.weight)
                patch: np.ndarray
                # compute p_k
